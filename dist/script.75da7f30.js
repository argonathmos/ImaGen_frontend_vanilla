...
},{"./lib/axios":"../node_modules/axios/lib/axios.js"}],"script.js":[function(require,module,exports) {
'use strict';
...
var randomImg = document.querySelector('.random-img'); // buttons
...
buttonsContainer.addEventListener('click', function (evt) {
  ...
  if (evt.target.nodeName === 'BUTTON') {
    ...
    message.textContent = "Waiting for ".concat(endpointName, " ...");
    var searchTerm = randomWord(evt.target.value.split(' '));
...
var getImage = function getImage(endpoint, searchTerm, endpointName) {
  _axios.default.get("https://imagen-rest-api.herokuapp.com/".concat(endpoint, "?q=").concat(searchTerm)).then(function (response) {
    ...
    setTimeout(function () {
      ...
      message.textContent = "".concat(endpointName, " says ").concat(searchTerm, " time!");
    }, 500);
  }).catch(function (error) {
    console.log(error);
    randomImg.style.display = 'None';
    message.textContent = "Sorry... ".concat(endpointName, " is in a bad mood today.");
  });
};
